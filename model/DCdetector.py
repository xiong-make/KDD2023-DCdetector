...
class DCdetector(nn.Module):
        ...
        self.encoder = Encoder(
            [
                AttentionLayer(
                    ...
                    d_model, patch_size, channel, n_heads, win_size) for _ in range(e_layers)
            ],
    ...
    def forward(self, x):
        B, L, M = x.shape  # Batch win_size channel (128, 105, 38)
        series_patch_mean = []
        prior_patch_mean = []
        ...
        # x_ori Batch win_size  channel (128, 105, 256)
        ...
        for patch_index, patchsize in enumerate(self.patch_size):
            ...
            # PyTorch 张量操作的函数，通常用于对张量的维度进行重新排列
